  ...
  try {
    ...
    const query = `
      INSERT INTO \`25정시결과\` (
        ...
        수학과목, 수학원점수, 수학표점, 수학백분위, 수학등급, 영어원점수, 영어등급, 탐1과목, 
        탐1원점수, 탐1표점, 탐1백분위, 탐1등급, 탐2과목, 탐2원점수, 탐2표점, 탐2백분위, 
        탐2등급, 한국사원점수, 한국사등급, 내신, 가_군, 가_대학명, 가_학과명, 가_수능, 
        가_내신, 가_실기, 가_총점, 가_최초결과, 가_최종결과, 가_실기종목1, 가1_기록, 가1_점수, 
        가_실기종목2, 가2_기록, 가2_점수, 가_실기종목3, 가3_기록, 가3_점수, 가_실기종목4, 가4_기록, 
        가4_점수, 가_실기종목5, 가5_기록, 가5_점수, 가_실기종목6, 가6_기록, 가6_점수, 나_군, 
        나_대학명, 나_학과명, 나_수능, 나_내신, 나_실기, 나_총점, 나_최초결과, 나_최종결과, 나_실기종목1, 
        나1_기록, 나1_점수, 나_실기종목2, 나2_기록, 나2_점수, 나_실기종목3, 나3_기록, 나3_점수, 
        나_실기종목4, 나4_기록, 나4_점수, 나_실기종목5, 나5_기록, 나5_점수, 나_실기종목6, 나6_기록, 
        나6_점수, 다_군, 다_대학명, 다_학과명, 다_수능, 다_내신, 다_실기, 다_총점, 다_최초결과, 다_최종결과, 
        다_실기종목1, 다1_기록, 다1_점수, 다_실기종목2, 다2_기록, 다2_점수, 다_실기종목3, 다3_기록, 
        다3_점수, 다_실기종목4, 다4_기록, 다4_점수, 다_실기종목5, 다5_기록, 다5_점수, 다_실기종목6, 
        다6_기록, 다6_점수
      ) VALUES ? 
      ON DUPLICATE KEY UPDATE 
        국어과목 = VALUES(국어과목), 국어원점수 = VALUES(국어원점수), 국어표점 = VALUES(국어표점), 
        국어백분위 = VALUES(국어백분위), 국어등급 = VALUES(국어등급), 수학과목 = VALUES(수학과목), 
        수학원점수 = VALUES(수학원점수), 수학표점 = VALUES(수학표점), 수학백분위 = VALUES(수학백분위), 
        수학등급 = VALUES(수학등급), 영어원점수 = VALUES(영어원점수), 영어등급 = VALUES(영어등급), 
        탐1과목 = VALUES(탐1과목), 탐1원점수 = VALUES(탐1원점수), 탐1표점 = VALUES(탐1표점), 
        탐1백분위 = VALUES(탐1백분위), 탐1등급 = VALUES(탐1등급), 탐2과목 = VALUES(탐2과목), 
        탐2원점수 = VALUES(탐2원점수), 탐2표점 = VALUES(탐2표점), 탐2백분위 = VALUES(탐2백분위), 
        탐2등급 = VALUES(탐2등급), 한국사원점수 = VALUES(한국사원점수), 한국사등급 = VALUES(한국사등급), 
        내신 = VALUES(내신)
    `;

    const values = data.map(row => [
      row['지점'], row['학교'], row['학년'], row['성별'], row['이름'], row['국어과목'], row['국어원점수'],
      row['국어표점'], row['국어백분위'], row['국어등급'], row['수학과목'], row['수학원점수'], row['수학표점'], 
      row['수학백분위'], row['수학등급'], row['영어원점수'], row['영어등급'], row['탐1과목'], row['탐1원점수'], 
      row['탐1표점'], row['탐1백분위'], row['탐1등급'], row['탐2과목'], row['탐2원점수'], row['탐2표점'], 
      row['탐2백분위'], row['탐2등급'], row['한국사원점수'], row['한국사등급'], row['내신'], row['가_군'], 
      row['가_대학명'], row['가_학과명'], row['가_수능'], row['가_내신'], row['가_실기'], row['가_총점'], 
      row['가_최초결과'], row['가_최종결과'], row['가_실기종목1'], row['가1_기록'], row['가1_점수'], 
      row['가_실기종목2'], row['가2_기록'], row['가2_점수'], row['가_실기종목3'], row['가3_기록'], row['가3_점수'], 
      row['가_실기종목4'], row['가4_기록'], row['가4_점수'], row['가_실기종목5'], row['가5_기록'], row['가5_점수'], 
      row['가_실기종목6'], row['가6_기록'], row['가6_점수'], row['나_군'], row['나_대학명'], row['나_학과명'], 
      row['나_수능'], row['나_내신'], row['나_실기'], row['나_총점'], row['나_최초결과'], row['나_최종결과'], 
      row['나_실기종목1'], row['나1_기록'], row['나1_점수'], row['나_실기종목2'], row['나2_기록'], row['나2_점수'], 
      row['나_실기종목3'], row['나3_기록'], row['나3_점수'], row['나_실기종목4'], row['나4_기록'], row['나4_점수'], 
      row['나_실기종목5'], row['나5_기록'], row['나5_점수'], row['나_실기종목6'], row['나6_기록'], row['나6_점수'], 
      row['다_군'], row['다_대학명'], row['다_학과명'], row['다_수능'], row['다_내신'], row['다_실기'], 
      row['다_총점'], row['다_최초결과'], row['다_최종결과'], row['다_실기종목1'], row['다1_기록'], row['다1_점수'], 
      row['다_실기종목2'], row['다2_기록'], row['다2_점수'], row['다_실기종목3'], row['다3_기록'], row['다3_점수'], 
      row['다_실기종목4'], row['다4_기록'], row['다4_점수'], row['다_실기종목5'], row['다5_기록'], row['다5_점수'], 
      row['다_실기종목6'], row['다6_기록'], row['다6_점수']
    ]);
