...
async function updateSusiData() {
  try {
    ...
    const values = data.map(row => [
      row.education_center || null, row.name || null, row.school || null, row.gender || '', row.grade || '', row.university || null,
      row.major || null, row.admission_type || '', row.score_converted || null, row.grade_level || null, row.other_info || null,
