...
// ✅ (신규) 전체 데이터 엑셀 다운로드 API
app.get('/26susi/all-excel-data', authJWT, async (req, res) => {
    try {
        // 1. 모든 지원자 데이터 조회
        // 주의: 최초합여부, 최종합여부 컬럼이 상담대학정보 테이블에 없으면 에러가 날 수 있음.
        // 확인 결과 live.html에서 사용 중이므로 어딘가에는 있음.
        const sql = `
            SELECT 
                u.대학명, u.학과명, u.전형명, u.실기ID,
                s.지점명, s.이름, s.성별,
                c.내신등급, c.내신점수, 
                c.기록1, c.점수1, c.기록2, c.점수2, c.기록3, c.점수3,
                c.기록4, c.점수4, c.기록5, c.점수5, c.기록6, c.점수6,
                ...
                c.최초합여부, c.최종합여부 
            FROM 상담대학정보 c
            JOIN 대학정보 u ON c.대학ID = u.대학ID
            JOIN 학생기초정보 s ON c.학생ID = s.학생ID
            ORDER BY u.대학명, u.학과명, u.전형명, s.지점명, s.이름
        `;

        const [rows] = await db.promise().query(sql);

        // 2. 실기ID별 종목명 매핑 (종목명 오름차순 기준)
        const [allEvents] = await db.promise().query("SELECT 실기ID, 종목명 FROM `26수시실기배점` ORDER BY 실기ID, 종목명");

        const eventMap = {};
        allEvents.forEach(r => {
            if (!eventMap[r.실기ID]) eventMap[r.실기ID] = [];
            // 최대 7개까지만 담음
            if (eventMap[r.실기ID].length < 7) {
                eventMap[r.실기ID].push(r.종목명);
            }
        });

        // 3. 결과 데이터에 종목명1~7 필드 추가
        const result = rows.map(row => {
            // 해당 학생이 지원한 전형(실기ID)의 종목 목록
            const events = eventMap[row.실기ID] || [];

            // row 객체에 종목명1, 종목명2... 추가
            for (let i = 0; i < 7; i++) {
                row[`종목명${i + 1}`] = events[i] || ''; // 없으면 빈 문자열
            }
            return row;
        });
        ...
        console.error("전체 엑셀 데이터 조회 오류:", err);
        res.status(500).json({ success: false, message: "서버 오류 (DB 컬럼 확인 필요: 최초합여부 등)" });
    }
});
